...
const createOrder = async (req, res) => {
    ...
    try {
        ...
        let parameter = {
            ...
            "transaction_details": {
                "order_id": "WWW-"+dataOrder.dataValues.id,
...
const setPaymentToExpired = async (req, res) => {
    ...
    try {
        ...
        while(retries > 0){
            // console.log(retries)
            try {
                transactionStatus = await coreApi.transaction.status(`WWW-${orderId}`);
                break;
        ...
        const formattedData = await Promise.all(data.map(async toko => {  
            ...
            const tokoData = toko?.dataValues;
                ...
                while(retries > 0){
                    // console.log(retries)
                    try {
                        transactionStatus = await coreApi.transaction.status(`WWW-${value.orderId}`);
                        break;
                        // console.log(transactionStatus)
                    } catch (error) {
                        retries--;
                ...
                for (const service of services) {
                    ...
                    for (const order of orders) {
                        ...
                        while(retries > 0){
                            // console.log(retries)
                            try {
                                transactionStatus = await coreApi.transaction.status(`WWW-${order.orders.dataValues.order_id}`);
                                break;
                                // console.log(transactionStatus)
                            } catch (error) {
    ...
    const checkStatus = async () =>{
        ...
        for(let order of getAllOrder){
            ...
            while(retries > 0){
                // console.log(retries)
                try {
                    transactionStatusOrder = await coreApiOrder.transaction.status(`WWW-${order.dataValues.id}`);
                    break;
                    // console.log(transactionStatus)
                } catch (error) {
                    if(error.httpStatusCode === '404'){
            ...
            for(let i = 0; i < dataOrder.length; i++){
                ...
                if(!detailOrder){
                    ...
                    while(retries > 0){
                        // console.log(retries)
                        try {
                            transactionStatus = await coreApi.transaction.status(`WWW-${dataOrder[i].dataValues.order_id}`);
                            break;
                            // console.log(transactionStatus)
                        } catch (error) {
                            retries--;
...
const getDetailOrderDokterPenyedia = async (req, res) =>{
    ...
    try {
        ...
        const dataDokter = await Dokter.findOne({
            attributes: ['nama', 'foto', ['harga', 'total_payment']],
            where:{
                id: detailOrder.dataValues.dokter_id
            }
        })
...
const getOrderStatusWaitingPaymentPenyediaDokterTrainer = async (req, res) =>{
    ...
    try {
        ...
        const isDokter = await Dokter.findOne({
            where:{
                penyedia_id: value.penyediaId
            }
        })

        const isTrainer = await Trainer.findOne({
            where:{
                penyedia_id: value.penyediaId
            }
        })
        ...
        if(isTrainer){
            const detailTrainer = await DetailOrderTrainer.findAll({
                where:{
                    trainer_id: isTrainer.dataValues.id
                }
            })

            for(let i = 0; i < detailTrainer.length; i++){
                const dataOrder = await Order.findOne({
                    attributes: [['id', 'order_id'], 'user_id', 'virtual_number', 'status_order', 'metode_pembayaran', 'status_pembayaran', 'tanggal_order'],
                    where:{
                        status_order:{
                        ...
                        id: detailTrainer[i].dataValues.order_id
                    }
                })

                dataOrder ? dataOrderTemp.push(dataOrder) : ''

            }

            if(dataOrderTemp.length === 0){
                return res.status(200).json({
                    response_code: 200,
                    message: "Tidak ada data"
                })
            } else {
                for(let i = 0; i < dataOrderTemp.length; i++){
                    const dataOrder1 = await Order.findOne({
                        where:{
                            id: dataOrderTemp[i].dataValues.order_id
                        }
                    })
                    ...
                    const userData = await User.findOne({
                        attributes: ['nama'],
                        where:{
                            id: dataOrderTemp[i].dataValues.user_id
                        }
                    })
                    ...
                    while(retries > 0){
                        // console.log(retries)
                        try {
                            transactionStatus = await coreApi.transaction.status(`WWW-${dataOrderTemp[i].dataValues.order_id}`);
                            break;
                        } catch (error) {
                            retries--;
                            if(retries === 0){
                    ...
                    if (transactionStatus.transaction_status === 'expire' && dataOrder1.dataValues.status_order === 'Waiting Payment') {
                        ...
                        await Chat.update({
                            ...
                            where: {
                                order_id: dataOrderTemp[i].dataValues.order_id
                            }
                        })
                    } else if(transactionStatus.transaction_status === 'settlement' && dataOrder1.dataValues.status_order === 'Waiting Payment'){
                        await dataOrder1.update({
                    ...
                    const orderDate = dataOrderTemp[i].dataValues.tanggal_order;
                    const diffInMilliseconds = now - orderDate; // Difference in milliseconds
                    let minutes = 0;
                    let seconds = 0
                    if (diffInMilliseconds < 3 * 60 * 1000) { // If less than 15 minutes
            ...
            const detailDokter = await DetailOrderDokter.findAll({
                where:{
                    dokter_id: isDokter.dataValues.id
                }
            })

            for(let i = 0; i < detailDokter.length; i++){
                const dataOrder = await Order.findOne({
                    attributes: [['id', 'order_id'], 'user_id', 'virtual_number', 'status_order', 'metode_pembayaran', 'status_pembayaran', 'tanggal_order'],
                    where:{
                        status_order:{
                        ...
                        id: detailDokter[i].dataValues.order_id
                    }
                })
...
const getOrderStatusOnProgressPenyediaDokterTrainer = async (req, res) =>{
...
const getOrderStatusCompleteExpireCancelPenyediaDokterTrainer = async (req, res) =>{
