...
const createOrder = async (req, res) => {
    ...
    try {
        ...
        let parameter = {
            ...
            "transaction_details": {
                "order_id": "YYY-"+dataOrder.dataValues.id,
...
const setPaymentToExpired = async (req, res) => {
    ...
    try {
        ...
        while(retries > 0){
            // console.log(retries)
            try {
                transactionStatus = await coreApi.transaction.status(`YYY-${orderId}`);
                break;
        ...
        const formattedData = await Promise.all(data.map(async toko => {  
            ...
            const tokoData = toko?.dataValues;
                ...
                while(retries > 0){
                    // console.log(retries)
                    try {
                        transactionStatus = await coreApi.transaction.status(`YYY-${value.orderId}`);
                        break;
                        // console.log(transactionStatus)
                    } catch (error) {
                        retries--;
                ...
                for (const service of services) {
                    ...
                    for (const order of orders) {
                        ...
                        while(retries > 0){
                            // console.log(retries)
                            try {
                                transactionStatus = await coreApi.transaction.status(`YYY-${order.orders.dataValues.order_id}`);
                                break;
                                // console.log(transactionStatus)
                            } catch (error) {
    ...
    const checkStatus = async () =>{
        ...
        for(let order of getAllOrder){
            ...
            while(retries > 0){
                // console.log(retries)
                try {
                    transactionStatusOrder = await coreApiOrder.transaction.status(`YYY-${order.dataValues.id}`);
                    break;
                    // console.log(transactionStatus)
                } catch (error) {
                    if(error.httpStatusCode === '404'){
            ...
            for(let i = 0; i < dataOrder.length; i++){
                ...
                if(!detailOrder){
                    ...
                    while(retries > 0){
                        // console.log(retries)
                        try {
                            transactionStatus = await coreApi.transaction.status(`YYY-${dataOrder[i].dataValues.order_id}`);
                            break;
                            // console.log(transactionStatus)
                        } catch (error) {
                            retries--;
            ...
            if(dataOrderTemp.length === 0){
                ...
                for(let i = 0; i < dataOrderTemp.length; i++){
                    ...
                    while(retries > 0){
                        // console.log(retries)
                        try {
                            transactionStatus = await coreApi.transaction.status(`YYY-${dataOrderTemp[i].dataValues.order_id}`);
                            break;
                        } catch (error) {
                            retries--;
                            if(retries === 0){
